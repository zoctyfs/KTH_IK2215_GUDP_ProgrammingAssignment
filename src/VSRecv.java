...
class ReceiveContext {
    ...
    public void processPacket(DatagramPacket packet) throws IOException {
        ...
        System.out.println("processPacket()  vspacket.type:"+vspacket.getType()+" state:"+state);
        if (debug) {
        ...
        if (vspacket.getType() == VSFtp.TYPE_BEGIN) {
            if ((state == State.NONE) || (state == State.CLOSED)) {
                ...
                System.out.println("processPacket() filename:"+filename+" localname:"+localname);
                state = State.OPEN;
            }
            else
            ...
            if (state == State.OPEN) {
                ...
                System.out.println("processPacket() outputStream:"+outputStream+" data:"+data);
            }
            else
    ...
    public void run() {
        while (true) {
            try {
                ...
                System.out.println("gUdpSocket.receive(packet)");
...
public class VSRecv {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("VSFTP Receiver start");
        receiver.start();
    }
}
